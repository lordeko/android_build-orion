...
"orionremote: Add a git remote for matching ORION repository"
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the ORION github
        T=$(gettop)
        pushd $T > /dev/null
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.orion.device=$ORION_BUILD");
    then
